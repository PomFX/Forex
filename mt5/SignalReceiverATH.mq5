   ...
   if(id == CHARTEVENT_KEYDOWN)
     {
      if((int)lparam == 116)
        {
         Print("[SignalReceiver] Manual refresh (F5)");
         if(g_mode==0)
            ProcessSignal();
         else
            RunOfflineBOS();
      ...
      if((int)lparam == 117)
        {
         Print("[SignalReceiver] Cancel all pending (F6)");
         CancelAllPending();
        }
      if((int)lparam == 118)
        {
   ...
   if(g_isBusy)
     {
      ...
      statusClr = C'255,200,50';
     }
   else
      if(tradeOk)
        {
         statusTxt = "ACTIVE";
         statusClr = C'50,220,100';
        }
      ...
      if(direction == "BUY" && slPrice >= price)
        {
         ...
         if(direction == "SELL" && slPrice <= price)
           {
            slPrice = NormalizeDouble(price + stopDist * 10, digits);
      ...
      if(direction == "BUY" && tpPrice <= price)
        {
         ...
         if(direction == "SELL" && tpPrice >= price)
           {
            tpPrice = NormalizeDouble(price - stopDist * 10, digits);
...
void MarkProcessed(int signalId)
  {
   if(g_processedCount >= 50)
     {
      for(int i = 0; i < 49; i++)
         g_processedIds[i] = g_processedIds[i+1];
      g_processedCount = 49;
     }
...
void ProcessOneSignal(string obj)
  {
   ...
   if(!allowed)
     {
      Print("[SignalReceiver] Skipped ", pair, " (filter)");
      return;
     }
   ...
   if(symbol == "")
     {
      Print("[SignalReceiver] Symbol not found: ", pair);
      return;
     }
...
int FindSwingPoints(double &highs[], double &lows[], int &highBars[], int &lowBars[])
  {
   ...
   ArrayResize(lows, 50);
   ArrayResize(highBars, 50);
   ArrayResize(lowBars, 50);
...
bool SendAccountInfo()
  {
   ...
   if(res != 200)
      Print("[SignalReceiver] AccountInfo response: ", CharArrayToString(result));
   return (res == 200);
  }
//+------------------------------------------------------------------+
